...
char* UUID4_PREFIX(create)() {
  ...
  const int capacity = UUID4_STR_BUFFER_SIZE + 1;

  char* buffer = calloc(capacity, sizeof(char));
  uuid4_to_s(uuid, buffer, capacity);
...
void UUID4_PREFIX(seed)(uint64_t* state) {
  ...
  *state = *state * 6364136223846793005u +
           ((uint64_t)(UUID4_PREFIX(mix)(UUID4_PREFIX(hash)(pid),
                                         UUID4_PREFIX(hash)(tid)))
            << 32);
...
void UUID4_PREFIX(seed)(uint64_t* state) {
  ...
  *state = *state * 6364136223846793005u +
           ((uint64_t)(UUID4_PREFIX(mix)(UUID4_PREFIX(hash)(pid),
                                         UUID4_PREFIX(hash)((uint32_t)tid)))
            << 32);
  *state = *state * 6364136223846793005u + (uintptr_t)getpid;
  *state = *state * 6364136223846793005u + (uintptr_t)UUID4_PREFIX(gen);
}
...
bool UUID4_PREFIX(to_s)(const UUID4_T uuid, char* out, int capacity) {
  ...
  if (capacity < UUID4_STR_BUFFER_SIZE) return false;
...
// $ gcc -O2 -Wall -Werror -DUUID4_PRACTPRAND_TEST -o uuid4_practrand_test
// uuid.c
...
int main(int argc, char* argv[]) {
  ...
  struct {
    const char* name;
    unsigned int (*gen)();
  } gens[] = {{"uuid4.dwords[0]", gen_uuid_0},
              {"uuid4.dwords[0] (reversed)", gen_uuid_0_rev},
              {"uuid4.dwords[1]", gen_uuid_1},
              {"uuid4.dwords[1] (reversed)", gen_uuid_1_rev},
              {"uuid4.dwords[2]", gen_uuid_2},
              {"uuid4.dwords[2] (reversed)", gen_uuid_2_rev},
              {"uuid4.dwords[3]", gen_uuid_3},
              {"uuid4.dwords[3] (reversed)", gen_uuid_3_rev}};

  for (size_t i = 0; i < sizeof(gens) / sizeof(gens[0]); ++i) {
    unif01_Gen* gen =
        unif01_CreateExternGenBits((char*)gens[i].name, gens[i].gen);
